...
const StartingBalanceModal: React.FC<StartingBalanceModalProps> = ({ 
  ...
  // CRITICAL: Scroll button into view on mobile
  useEffect(() => {
    if (watchedBalance && watchedBalance >= 100 && buttonRef.current) {
      const timer = setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] bg-black bg-opacity-50 backdrop-blur-sm flex items-start justify-center p-4 overflow-y-auto">
      <div 
        ref={modalRef}
        className="relative bg-white dark:bg-gray-800 rounded-xl w-full max-w-lg border border-gray-200 dark:border-gray-700 shadow-2xl animate-fade-in-scale my-4 min-h-fit"
      >
        {/* Header */}
        ...
        {/* CRITICAL: Action Buttons - MOVED TO BOTTOM & ALWAYS VISIBLE */}
        <div className="sticky bottom-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 p-4 sm:p-6 rounded-b-xl">
