...
const AddTradeModal: React.FC<AddTradeModalProps> = ({ 
  ...
  // Auto-focus first input and scroll into view when modal opens
  useEffect(() => {
    const timer = setTimeout(() => {
      // Scroll modal into view first
      if (modalRef.current) {
  ...
  // Prevent body scroll when modal is open and ensure proper z-index
  useEffect(() => {
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 overflow-y-auto">
      ...
      <div 
        ref={modalRef}
        className="relative bg-white dark:bg-gray-800 rounded-xl max-w-4xl w-full max-h-[95vh] overflow-y-auto border border-gray-200 dark:border-gray-700 shadow-2xl animate-fade-in-scale my-8"
        style={{ 
